...
def won_lost():
    ...
    main_surface.blit(result_font.render(s, True, YELLOW), (170, 200) )

    # prompt_font = pygame.font.SysFont('monospace', 24)
    # prompt_font.set_bold(True)
    # main_surface.blit(prompt_font.render("Натисни ПРОБІЛ, щоб грати ще", True, GREEN), (200, 300) )
...
player_score = pygame.mixer.Sound("player_score.wav")
enemy_score_sound = pygame.mixer.Sound("enemy_score.wav")
enemy_collide = pygame.mixer.Sound("enemy_collide.wav")
...
while is_working:
    ...
    for enemy in enemies:
        ...
        # If the same enemy collides both with the left border and the player
            ...
            enemies.pop(enemies.index(enemy))
            score -= 1
            pygame.mixer.Sound.play(enemy_collide)
            # pygame.mixer.music.stop()
            if score < 0:
                game_over = True
                score = 0
    ...
    for bonus in bonuses:
        ...
        if bonus.get_right() < 0 or bonus.get_bottom() < 0 or bonus.get_left() > width or bonus.get_top() > height:
            ...
            bonuses.pop(bonuses.index(bonus))
            score += 1
            pygame.mixer.Sound.play(player_score)
            # pygame.mixer.music.stop()
            ...
            for enemy in enemies:
                if bonus.collides(enemy):
                    if bonus in bonuses:
                        bonuses.pop(bonuses.index(bonus))
                        enemy_score += 1
                        pygame.mixer.Sound.play(enemy_score_sound)
                        # pygame.mixer.music.stop()
